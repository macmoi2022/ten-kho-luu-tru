...
function getReports($vuaDir, $filterDate_dmy) {
    ...
    foreach ($raw_header as $h) {
        $header[] = trim($h); // Giữ nguyên case để match chính xác
    }
    ...
    foreach ($lines as $line) {
        ...
        if (count($header) == count($row)) {
            ...
            // Xử lý trường hợp file cũ thiếu cột
            $temp_report = [];
            foreach ($header as $index => $key) {
                $temp_report[$key] = $row[$index] ?? null;
            }
            $reports[] = $temp_report;
        }
    }
...
function createReportSheet(Spreadsheet &$spreadsheet, array $reports, int $sheetIndex, string $sheetName, bool $isToaDiBanSheet = false) {
    if ($sheetIndex === 0) {
    ...
    if (empty($reports)) {
        ...
        foreach ($reports as $report) {
            ...
            $totalKg = getValue($report['Tổng_KG'] ?? 0);
            $thanhTien = getValue($report['Thành_Tiền'] ?? 0);
            ...
            $totals['Thành_Tiền'] += $thanhTien;

            $rowData = array_merge($rowData, [$totalKg ?: '', $thanhTien ?: '', $report['Ghi_Chú'] ?? '', (($report['isPaid'] ?? 'false') == 'true' ? 'Đã CK' : '')]);
...
function createSummarySheet(Spreadsheet &$spreadsheet, array $reports7, array $reports9) {
    ...
    foreach ($allReports as $vuaKey => $reports) {
        ...
        foreach ($reports as $report) {
            ...
            $stats[$vuaKey]['totalAmount'] += getValue($report['Thành_Tiền'] ?? 0);
        }
    }
...
if ($type === 'quanly') {
    $allReports7 = getReports('Data_vua7/', $date);
    $allReports9 = getReports('Data_vua9/', $date);

    $normalReports7 = array_filter($allReports7, fn($r) => ($r['toaDiBan'] ?? 'false') !== 'true');
    $toaDiBanReports7 = array_filter($allReports7, fn($r) => ($r['toaDiBan'] ?? 'false') === 'true');
    
    $normalReports9 = array_filter($allReports9, fn($r) => ($r['toaDiBan'] ?? 'false') !== 'true');
    $toaDiBanReports9 = array_filter($allReports9, fn($r) => ($r['toaDiBan'] ?? 'false') === 'true');

    createReportSheet($spreadsheet, $normalReports7, $nextSheetIndex++, 'Báo Cáo Vựa 7');
    createReportSheet($spreadsheet, $normalReports9, $nextSheetIndex++, 'Báo Cáo Vựa 9');
    
    if (!empty($toaDiBanReports7)) {
        createReportSheet($spreadsheet, $toaDiBanReports7, $nextSheetIndex++, 'TOA BÁN - Vựa 7', true);
    }
    if (!empty($toaDiBanReports9)) {
        createReportSheet($spreadsheet, $toaDiBanReports9, $nextSheetIndex++, 'TOA BÁN - Vựa 9', true);
    }

    createSummarySheet($spreadsheet, $normalReports7, $normalReports9);
    $spreadsheet->setActiveSheetIndex(0);

} else {
    if (!empty($vua) && in_array($vua, ['Data_vua7/', 'Data_vua9/'])) {
        $vuaName = ($vua === 'Data_vua7/') ? 'Vựa 7' : 'Vựa 9';
        $allReports = getReports($vua, $date);
        
        $normalReports = array_filter($allReports, fn($r) => ($r['toaDiBan'] ?? 'false') !== 'true');
        $toaDiBanReports = array_filter($allReports, fn($r) => ($r['toaDiBan'] ?? 'false') === 'true');

        createReportSheet($spreadsheet, $normalReports, $nextSheetIndex++, 'Báo Cáo ' . $vuaName);
        if (!empty($toaDiBanReports)) {
            createReportSheet($spreadsheet, $toaDiBanReports, $nextSheetIndex++, 'TOA BÁN - ' . $vuaName, true);
        }
        $spreadsheet->setActiveSheetIndex(0);
    }
}
