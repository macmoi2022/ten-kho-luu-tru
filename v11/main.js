...
function updateCalculationPreview(inputElement) {
    ...
    if ((value.includes('+') || value.includes('-')) && /[0-9]/.test(value)) {
            ...
            <div class="flex justify-end">
                <div class="w-full grand-total">
                     <div class="flex justify-between items-center py-2 border-t-2 border-b-2 border-gray-800" style="padding-bottom: 5mm;">
                        ...
                        <span class="total-amount font-bold text-indigo-600 whitespace-nowrap text-right">${(calculateFinalAmount(report)).toLocaleString('vi-VN')} VND</span>
                    </div>
                </div>
            </div>
            <div class="flex justify-end">
...
function exportStatsToExcel() {
    ...
    function getValueFromExpression(value) {
        ...
        // Replace multiple +/- with a single one, and handle starting operator
        const cleanedValue = value.replace(/\s/g, '').replace(/--/g, '+').replace(/\+\+/g, '+').replace(/-\+/g, '-').replace(/\+-/g, '-');
        
        // Match numbers with optional preceding +/-
    ...
    function calculateRowTotals() {
        ...
        const tempReport = {};
        tempReport.khongThanhToan = document.getElementById('khongThanhToan').checked;
        tempReport.tuyChonTien = document.getElementById('tuyChonTien').value;
        ...
        quantityColumns.forEach(qtyColId => {
            ...
            if(qtyEl && priceEl) {
                ...
                totalKG += kg;
                tempReport[qtyColId] = qtyEl.value;
                tempReport[`Giá_${qtyColId}`] = priceEl.value;
            }
        });

        const finalAmount = calculateFinalAmount(tempReport);
    ...
    function bindFormToReport(report) {
        ...
        document.getElementById('tuyChonTien').value = report.tuyChonTien || '';
        document.getElementById('ghiChuTuyChon').value = report.ghiChuTuyChon || '';

        // Handle title change for "Toa đi bán"
    ...
    function updateLiveReportFromForm() {
        ...
        liveEditingReport.tuyChonTien = document.getElementById('tuyChonTien').value;
    ...
    function handleFormInput(e) {
        ...
        const isCalcInput = allCalcColumns.includes(input.id) || input.id === 'tuyChonTien';

        if (isCalcInput) {
            let value = input.value;
            // Allow numbers, +, and - at the beginning
        ...
        // Update title if 'toaDiBan' checkbox is changed
    ...
    function renderStatsTable(stats) {
        ...
        // *** MODIFIED: Added new row for Thành Tiền per Vựa ***
