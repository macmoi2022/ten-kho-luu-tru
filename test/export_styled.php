...
require 'spread-sheet-library/autoload.php';
...
function getValueFromExpression($value) {
    if (!is_string($value) || trim($value) === '') return 0.0;
...
function calculateFinalAmount($report, $quantityColumns) {
    if (($report['khongthanhtoan'] ?? 'false') === 'true') {
        return 0;
    }
    $baseAmount = 0;
    foreach ($quantityColumns as $col) {
        $qty = getValueFromExpression($report[$col] ?? '0');
        $price = getValueFromExpression($report['giá_' . $col] ?? '0');
...
function getReports($vuaDir, $filterDate_dmy) {
    ...
    foreach ($lines as $line) {
        ...
        if (count($header) == count($row)) {
            $report = array_combine($header, $row);
            
            // Tính toán lại Thành Tiền và Tổng KG để đảm bảo chính xác
            $report['thành_tiền'] = calculateFinalAmount($report, $quantityColumns);
            
            $totalKg = 0;
            foreach($quantityColumns as $col) {
                $totalKg += getValueFromExpression($report[$col] ?? '0');
            }
            $report['tổng_kg'] = $totalKg;
...
function populateSheetWithData(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet &$sheet, array $reports, array $headerForDisplay, array $quantityColumns) {
    ...
    if (empty($reports)) {
        ...
        foreach ($reports as $report) {
            $totalKg = $report['tổng_kg'];
            $totalAmount = ($report['toadiban'] ?? 'false') === 'true' ? calculateFinalAmount($report, $quantityColumns) : $report['thành_tiền'];
            
            $lai_data = $report['lái'] ?? '';
            $rowData = [$report['toa'] ?? '', mb_strtoupper($lai_data, 'UTF-8')];
            
            foreach ($quantityColumns as $qc) {
                $qty = getValueFromExpression($report[$qc] ?? 0);
                $price = getValueFromExpression($report['giá_'.$qc] ?? 0);
                $rowData[] = $qty ?: '';
                $rowData[] = $price ?: '';
                $totals[$qc] += $qty;
            }
...
function createReportAndSellingSheets(Spreadsheet &$spreadsheet, array $allReports, int $startIndex, string $baseName) {
    global $quantityColumns;
    $normalReports = array_filter($allReports, function($r) { return ($r['toadiban'] ?? 'false') !== 'true'; });
    $sellingReports = array_filter($allReports, function($r) { return ($r['toadiban'] ?? 'false') === 'true'; });

    $sheet = $startIndex === 0 ? $spreadsheet->getSheet(0) : $spreadsheet->createSheet();
    ...
    populateSheetWithData($sheet, $normalReports, $headerForDisplay, $quantityColumns);

    if (!empty($sellingReports)) {
        $sellingSheet = $spreadsheet->createSheet();
        $sellingSheet->setTitle("TOA BÁN - {$baseName}");
        populateSheetWithData($sellingSheet, $sellingReports, $headerForDisplay, $quantityColumns);
        return 2;
    }
    return 1;
}
...
function createSummarySheet(Spreadsheet &$spreadsheet, array $reports7, array $reports9) {
    ...
    $stats = ['vua7' => ['details' => [], 'totalAmount' => 0, 'totalKg' => 0], 'vua9' => ['details' => [], 'totalAmount' => 0, 'totalKg' => 0]];
    ...
    foreach ($allReports as $vuaKey => $reports) {
        ...
        foreach ($reports as $report) {
            if (($report['toadiban'] ?? 'false') === 'true') continue;
            
            $stats[$vuaKey]['totalAmount'] += $report['thành_tiền'];
            
            foreach ($quantityColumns as $col) {
                $qty = getValueFromExpression($report[$col] ?? 0);
                $stats[$vuaKey]['details'][$col]['kg'] += $qty;
                $stats[$vuaKey]['totalKg'] += $qty;
            }
        }
    }
    ...
    $sheet->fromArray(['THÀNH TIỀN', $stats['vua7']['totalAmount'] ?: '', $stats['vua9']['totalAmount'] ?: ''], NULL, 'A'.$currentRow)->getStyle('A'.$currentRow.':C'.$currentRow)->applyFromArray($grandTotalStyle);
    $sheet->getStyle('B'.$currentRow.':C'.$currentRow)->getNumberFormat()->setFormatCode('#,##0" đ"');
    
    $lastStyledRow = $currentRow;

    if($stats['vua7']['totalAmount'] > 0 && $stats['vua9']['totalAmount'] > 0) {
        $currentRow++;
        ...
        $sheet->getStyle('B'.$currentRow)->getNumberFormat()->setFormatCode('#,##0" đ"');
        $lastStyledRow = $currentRow;
    }

    $sheet->getStyle('B2:C'.$lastStyledRow)->applyFromArray($numberFormatStyle);
    ...
    $allBordersStyle = ['borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]]];
    if ($lastStyledRow > 1) { 
        $sheet->getStyle('A1:C' . $lastStyledRow)->applyFromArray($allBordersStyle);
    }
    for ($i = 2; $i <= $lastStyledRow; $i++) {
        $sheet->getRowDimension($i)->setRowHeight(16.5);
    }
}
...
if ($type === 'quanly') {
    ...
    $sheetIndex = 0;
    $sheetIndex += createReportAndSellingSheets($spreadsheet, $reports7, $sheetIndex, 'Vựa 7');
    $sheetIndex += createReportAndSellingSheets($spreadsheet, $reports9, $sheetIndex, 'Vựa 9');
    ...
    if (!empty($vua) && in_array($vua, ['Data_vua7/', 'Data_vua9/'])) {
        ...
        createReportAndSellingSheets($spreadsheet, $reports, 0, $vuaName);
    }
}
