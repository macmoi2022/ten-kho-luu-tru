...
const reportExtraFields = ['khongThanhToan', 'toaDiBan', 'tuyChonTien', 'ghiChuTuyChon'];


// --- CÁC HÀM TIỆN ÍCH ---

/**
 * Định dạng một chuỗi số hoặc biểu thức với dấu chấm phân cách hàng nghìn.
 * @param {string} value - Chuỗi đầu vào (ví dụ: "1000+500").
 * @returns {string} Chuỗi đã được định dạng (ví dụ: "1.000+500").
 */
function formatNumberInput(value) {
    if (typeof value !== 'string' || value.trim() === '') return '';
    // Tách chuỗi theo các toán tử (+, -) để định dạng riêng từng số
    const parts = value.split(/([+\-])/);
    const formattedParts = parts.map(part => {
        if (part === '+' || part === '-' || part === '') {
            return part; // Giữ nguyên toán tử
        }
        // Thêm dấu chấm vào chuỗi số
...
/**
 * Loại bỏ các dấu chấm phân cách hàng nghìn khỏi chuỗi.
 * @param {string} value - Chuỗi đầu vào (ví dụ: "1.234.567").
 * @returns {string} Chuỗi số không có dấu chấm (ví dụ: "1234567").
 */
...
function updateCalculationPreview(inputElement) {
    const isCalcInput = allCalcColumns.includes(inputElement.id) || inputElement.id === 'tuyChonTien';
    ...
    if (inputElement.id === 'tuyChonTien') {
        // Đối với ô "Tùy chọn tiền", chỉ hiển thị khi có 2 toán tử trở lên
        shouldShow = operatorCount >= 2;
    } else {
        // Đối với các ô khác, giữ nguyên logic cũ: 1 toán tử và có số
        shouldShow = operatorCount >= 1 && /[0-9]/.test(value);
    }
...
function showAllCalculationPreviews() {
    [...allCalcColumns, 'tuyChonTien'].forEach(id => {
...
async function showInvoice(reportIndex) {
    ...
    let customerName = report.driver.toUpperCase();
    let toaBanHtml = '';
    if (report.toaDiBan) {
        toaBanHtml = `<div class="text-center font-bold text-red-600 text-2xl my-2">(TOA BÁN)</div>`;
    }
    ...
    let tuyChonTienHtml = '';
    const tuyChonTienValue = report.tuyChonTien || '';
    const ghiChuTuyChon = report.ghiChuTuyChon || '';

    if (tuyChonTienValue.trim() !== '') {
        const adjustment = getValueFromExpression(tuyChonTienValue);
        const formattedExpression = formatNumberInput(tuyChonTienValue);
        
        let calculationString = formattedExpression;
        
        const operatorCount = (tuyChonTienValue.match(/[+\-]/g) || []).length;

        if (operatorCount >= 2) {
            calculationString += `=${adjustment.toLocaleString('vi-VN')}`;
        }

        const noteString = ghiChuTuyChon ? ` (${ghiChuTuyChon})` : '';

        tuyChonTienHtml = `
            <div class="flex justify-between items-center py-1 border-t border-gray-300" style="padding-bottom: 3mm; padding-top: 3mm;">
                <span class="text-gray-700 whitespace-nowrap"><strong>Tùy chọn thêm/bớt:</strong></span>
                <span class="font-medium text-gray-900 whitespace-nowrap text-right">${calculationString}${noteString}</span>
            </div>
        `;
    }

    const finalAmountText = report.khongThanhToan 
        ? `<span class="font-bold text-red-600 mx-2">(Không Thanh Toán)</span> 0 VND`
        : `${(calculateFinalAmount(report)).toLocaleString('vi-VN')} VND`;
            ...
            <div class="flex justify-end">
                <div class="w-full grand-total">
                     ...
                     <div class="flex justify-between items-center py-2 border-t-2 border-b-2 border-gray-800" style="padding-bottom: 5mm;">
                        ...
                        <span class="total-amount font-bold text-indigo-600 whitespace-nowrap text-right">${finalAmountText}</span>
                    </div>
                </div>
            </div>
            <div class="flex justify-end">
...
async function showPaymentModal(reportIndex) {
    ...
    const report = dailyReports[reportIndex];
    if (!report) return;
    if (report.khongThanhToan || report.toaDiBan) {
        return customAlert('Không thể thanh toán cho loại toa này.');
    }
    ...
    const amount = calculateFinalAmount(report);
    ...
    if (report.isPaid) {
        ...
        if (currentIndexInDay !== -1) {
            for (let i = currentIndexInDay + 1; i < reportsForDay.length; i++) {
                if (!reportsForDay[i].isPaid && !reportsForDay[i].khongThanhToan && !reportsForDay[i].toaDiBan) {
...
function exportStatsToExcel() {
    ...
    function handleDriverSelection(selectedDriver) {
        ...
        if (liveEditingReport) {
            ...
            reportExtraFields.forEach(field => {
                newReport[field] = '';
            });
    ...
    function getValueFromExpression(value) {
        ...
        const cleanedValue = unformattedValue.replace(/\s/g, '').replace(/--/g, '+').replace(/\+\+/g, '+').replace(/-\+/g, '-').replace(/\+-/g, '-');
        
        const parts = cleanedValue.match(/[+-]?[0-9.]+/g);
        if (!parts) return 0;
    ...
    function calculateRowTotals() {
        ...
        quantityColumns.forEach(qtyColId => {
            ...
            if (qtyEl && priceEl) {
                const kg = getValueFromExpression(qtyEl.value);
        ...
        const tuyChonTienValue = document.getElementById('tuyChonTien').value;
        const adjustment = getValueFromExpression(tuyChonTienValue);
        const isKhongThanhToan = document.getElementById('khongThanhToan').checked;
        const finalAmount = isKhongThanhToan ? 0 : baseAmount + adjustment;

        const tongKgDisplay = totalKG > 0 ? totalKG.toLocaleString('vi-VN') : '0';
        const thanhTienDisplay = finalAmount.toLocaleString('vi-VN');
        ...
        const breakdownEl = document.getElementById('sticky-total-tien-breakdown');
        
        const hasTuyChonTien = tuyChonTienValue.trim() !== '';

        if (isKhongThanhToan) {
            breakdownEl.textContent = '(Không Thanh Toán)';
            breakdownEl.style.display = 'block'; 
        } else if (hasTuyChonTien) {
            const operator = adjustment >= 0 ? '+' : '-';
            const absAdjustment = Math.abs(adjustment);
            breakdownEl.textContent = `(${baseAmount.toLocaleString('vi-VN')} ${operator} ${absAdjustment.toLocaleString('vi-VN')})`;
            breakdownEl.style.display = 'block';
        } else {
            breakdownEl.textContent = '';
            breakdownEl.style.display = 'none'; // Ẩn div khi không có điều kiện
        }
    }
    ...
    function lockForm(shouldLock) {
        ...
        // Also lock/unlock the new options
        document.getElementById('khongThanhToan').disabled = shouldLock;
        document.getElementById('toaDiBan').disabled = shouldLock;
        document.getElementById('tuyChonTien').disabled = shouldLock;
        document.getElementById('ghiChuTuyChon').disabled = shouldLock;
    }
    ...
    function bindFormToReport(report) {
        ...
        allCalcColumns.forEach(col => {
            ...
            if (input) {
                input.value = formatNumberInput(report[col] || '');
            }
        });

        document.getElementById('khongThanhToan').checked = report.khongThanhToan || false;
        document.getElementById('toaDiBan').checked = report.toaDiBan || false;
        document.getElementById('tuyChonTien').value = formatNumberInput(report.tuyChonTien || '');
        document.getElementById('ghiChuTuyChon').value = report.ghiChuTuyChon || '';

        const titleEl = document.getElementById('add-report-title');
        let titleText = `Sửa Dữ Liệu Toa ${report.toa}`;
        if (report.toaDiBan) {
            titleText += " (TOA BÁN)";
        }
    ...
    function updateLiveReportFromForm() {
        ...
        allCalcColumns.forEach(col => {
            ...
            if (input) {
                liveEditingReport[col] = unformatNumberInput(input.value);
            }
        });

        liveEditingReport.khongThanhToan = document.getElementById('khongThanhToan').checked;
        liveEditingReport.toaDiBan = document.getElementById('toaDiBan').checked;
        liveEditingReport.tuyChonTien = unformatNumberInput(document.getElementById('tuyChonTien').value);
        liveEditingReport.ghiChuTuyChon = document.getElementById('ghiChuTuyChon').value;

        liveEditingReport['Tổng_KG'] = calculateTotalKG(liveEditingReport);
        liveEditingReport['Thành_Tiền'] = calculateFinalAmount(liveEditingReport);
    }
    ...
    function handleFormInput(e) {
        ...
        const isNumericField = [...allCalcColumns, 'tuyChonTien'].includes(input.id);
        ...
        if (e.target.id === 'toaDiBan') {
            const titleEl = document.getElementById('add-report-title');
            let titleText = `Sửa Dữ Liệu Toa ${liveEditingReport.toa}`;
            if (e.target.checked) {
                titleText += " (TOA BÁN)";
            }
            titleEl.textContent = titleText;
    ...
    function renderDailyReportTable(reports, expandedToaToRestore = new Set()) {
        ...
        if (isMobile) {
            ...
            reports.forEach((report) => {
                ...
                if (userType === 'quanly' && !report.khongThanhToan && !report.toaDiBan) {
                ...
                const totalAmount = calculateFinalAmount(report);
                        ...
                        <div class="flex-grow">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <span class="lai">${laiHtml}${verifiedIconHtml}</span>
                                </div>
                                <span class="mobile-report-toggle-icon">▼</span>
                            </div>
                            <div class="flex justify-around items-center text-sm mt-2 pt-2 border-t border-gray-100">
                                <div class="text-center">
                                ...
                                ${report.toaDiBan ? '<div class="text-center"><span class="font-bold text-red-600 text-xs">(TOA BÁN)</span></div>' : ''}
                                <div class="text-center">
            ...
            reports.forEach((report) => {
                ...
                let laiHtml = report.driver;
                if (report.toaDiBan) {
                    laiHtml += ' <span class="font-bold text-red-600">(TOA BÁN)</span>';
                }
    ...
    function resetFormForNewEntry() {
        ...
        // Reset new fields
        document.getElementById('khongThanhToan').checked = false;
        document.getElementById('toaDiBan').checked = false;
        document.getElementById('tuyChonTien').value = '';
        document.getElementById('ghiChuTuyChon').value = '';
        document.getElementById('add-report-title').textContent = 'Thêm Dữ Liệu Báo Cáo';

        // Reset and hide the options panel
        document.getElementById('sticky-options-panel').classList.add('hidden');
        document.getElementById('sticky-options-toggle').querySelector('span').textContent = '︽';

        const calculationResults = document.querySelectorAll('#dailyReportForm .calculation-result, #sticky-options-panel .calculation-result');
    ...
    async function loadDailyReports() {
        try {
            ...
            const result = await response.json();
            if (result && result.success) {
                dailyReports = result.data.map(r => ({
                     ...r, 
                     isPaid: r.isPaid === 'true',
                     khongThanhToan: r.khongThanhToan === 'true',
                     toaDiBan: r.toaDiBan === 'true'
                    })) || [];
                filterReportsByDate();
            } else {
    ...
    function calculateBaseTotalAmount(report) {
    ...
    function calculateFinalAmount(report) {
        if (report.khongThanhToan) {
            return 0;
        }
        const baseAmount = calculateBaseTotalAmount(report);
        const adjustment = getValueFromExpression(report.tuyChonTien);
    ...
    function updateSummaryTable(reports) {
        ...
        const reportsForSummary = reports.filter(r => !r.toaDiBan);

        if (reportsForSummary.length === 0) {
        ...
        reportsForSummary.forEach(report => {
            const finalAmount = calculateFinalAmount(report);
            const baseAmount = calculateBaseTotalAmount(report);
            const ratio = baseAmount > 0 ? finalAmount / baseAmount : 0;

            quantityColumns.forEach(qtyCol => {
                const kg = getValueFromExpression(report[qtyCol]);
                const price = getValueFromExpression(report[`Giá_${qtyCol}`]);
                const amount = (kg * price) * (report.khongThanhToan ? 0 : ratio);
    ...
    async function saveDailyReportsToCSV() {
        try {
            const reportsToSave = dailyReports.map(report => {
                ...
                reportCopy.khongThanhToan = report.khongThanhToan ? 'true' : 'false';
                reportCopy.toaDiBan = report.toaDiBan ? 'true' : 'false';
                
                const { Tổng_KG, Thành_Tiền, ...rest } = reportCopy;
                const finalReport = {
                    ...rest,
                    Tổng_KG: calculateTotalKG(reportCopy),
                    Thành_Tiền: calculateFinalAmount(reportCopy)
                };
                return finalReport;
            });
    ...
    function renderStatsTable(stats) {
        ...
        // --- HÀNG THÁI ---
        let totalThaiKg7 = 0, totalThaiKg9 = 0;
        thaiTypeOrder.forEach(type => {
        ...
        // --- HÀNG INDO ---
        let totalIndoKg7 = 0, totalIndoKg9 = 0;
        indoTypeOrder.forEach(type => {
            const detail7 = vua7Data.details[type] || { kg: 0 };
            const detail9 = vua9Data.details[type] || { kg: 0 };
        ...
        totalAmountRow.innerHTML = `
            <td class="px-2 sm:px-6 py-4 text-center">THÀNH TIỀN</td>
            <td class="px-2 sm:px-6 py-4 text-center">${vua7Data.total_amount > 0 ? vua7Data.total_amount.toLocaleString('vi-VN') + ' đ' : ''}</td>
            <td class="px-2 sm:px-6 py-4 text-center">${vua9Data.total_amount > 0 ? vua9Data.total_amount.toLocaleString('vi-VN') + ' đ' : ''}</td>
        `;

        if (vua7Data.total_amount > 0 && vua9Data.total_amount > 0) {
            const grandTotalAmount = (vua7Data.total_amount || 0) + (vua9Data.total_amount || 0);
            const grandTotalAmountRow = tableBody.insertRow();
            grandTotalAmountRow.className = `font-extrabold text-purple-800 bg-purple-100`;
	...
	document.addEventListener("DOMContentLoaded", async () => {
        ...
        [...allCalcColumns, 'tuyChonTien'].forEach(id => {
			const input = document.getElementById(id);
			if(input) {
                // Ensure the input is not already wrapped
        ...
        document.getElementById('sticky-options-panel').addEventListener('input', handleFormInput);
        ...
        document.getElementById('sticky-options-toggle').addEventListener('click', () => {
            const panel = document.getElementById('sticky-options-panel');
            const icon = document.getElementById('sticky-options-toggle').querySelector('span');
